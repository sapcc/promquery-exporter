...
build-all: build/promquery

build/promquery: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/promquery .
...
install: FORCE build/promquery
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/promquery "$(DESTDIR)$(PREFIX)/bin/promquery"
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/sapcc/promquery-exporter\e[0m\n"
	@goimports -w -local github.com/sapcc/promquery-exporter.git $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/promquery\e[0m              Build promquery.\n"
